...
def main():
    ...
    parser.add_option("-s", "--solv-ff", dest="solv_ff", type="string", default="tip3p",
        help="Which water model to use. Or solvent model, but that would be untested.")
    ...
    cmplx = u.select_atoms('{}'.format(options.cmplx_sel))
    ...
    with open('solv_box.leap','w') as f:
        ...
        f.write('source leaprc.water.{}\n'.format(options.solv_ff))
        f.write('source leaprc.protein.ff14SB\n')
    
        f.write('system = loadpdb trunc_a.pdb\n')

        f.write('solvateoct system {}BOX 0.0\n'.format(options.solv_ff.upper()))
